...
out_path = "output/optimisations/optim_2sphere_1view/"
if len(sys.argv) > 1:
    out_path += sys.argv[1] + "/"
...
def create_scene(sdf_file, interpolation, cams_origins, fov, cam_res):
    ...
    sdf = """
        <sdf id="SDF" type="sdf_explicit">
            <integer name="sphere_tracing_steps" value="200"/>
...
for i in range(len(cams_origins)):
    ref = 0
    for _ in range(3):
        ref += render_torch(scene_target, spp=spp_ref, sensor_index=i)
    images_ref.append(ref / 3)
...
objective = BoxPyramidLoss(images_ref, nn.MSELoss(reduction="mean"), reduction='mean')
